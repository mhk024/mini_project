...
No heavy imports (torch, sentence‑transformers, chromadb) at module import time.
Models load lazily inside get_*() functions, protected by a lock for thread safety.
"""
...
# ---------- Embeddings ----------
def get_embeddings():
    """Singleton embedding model (FastEmbed by default, HuggingFace fallback)."""
    global embeddings
...
# ---------- LLM (Groq) ----------
def get_llm():
    """Singleton Groq LLM client (created on first RAG/analysis request)."""
    global _llm
    if _llm is not None:
        return _llm
...
def release_heavy_models(keep_embeddings: bool = True):
    ...
    global _llm, _cross_encoder, embeddings
    with _lock:
        _llm = None
        _cross_encoder = None
